...
namespace CommonTools
{
	...
	public class CellPainter
	{
			...
			object data)
		{
			if (format.BackColor != Color.Transparent)
			{
                ...
                if (!format.BackColorIsFullRow && column .Index==0)
                {                 }
                else
                {
			...
			if (data != null)
			{
                ...
                if(node is CheckNode && column.Index == 0)
                {
                    var cknode = (CheckNode)node;
                    int h = TextRenderer.MeasureText(str, format.Font).Height;
                    if (cknode.IsChecked)
                        ControlPaint.DrawCheckBox(dc, cellRect.X, cellRect.Y, h, h, ButtonState.Checked);
                    else
                        ControlPaint.DrawCheckBox(dc, cellRect.X, cellRect.Y, h, h, ButtonState.Normal);
                    cellRect.X += h;
                    cellRect.Width -= h;
                }
                TextRenderer.DrawText(dc, str, format.Font, cellRect, color, flags);
			}
		}
	...
	public class RowPainter
	{
        ...
        public void DrawHeader(Graphics dc, Rectangle r, bool isHot, Image headerIcon = null)
        {
            if (!Application.RenderWithVisualStyles)
            {
                ControlPaint.DrawButton(dc, r, ButtonState.Flat);
            ...
            if (headerIcon != null)
            {
                ...
                headerRect.X =r.X+ r.Width / 2 - headerIcon.Width / 2;
                headerRect.Y =r.Y+ r.Height / 2 - (headerIcon.Height / 2);
                headerRect.Width = headerIcon.Width;
                headerRect.Height = headerIcon.Height;
                dc.DrawImageUnscaled(headerIcon, headerRect);
            }
        }
