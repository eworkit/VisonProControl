...
{
        public delegate void TreeViewListEventHandler(object sender, TreeListViewEventArgs args);
	[Designer(typeof(TreeListViewDesigner))]
	public class TreeListView : Control, ISupportInitialize
	{
		public event TreeViewListEventHandler AfterSelect;
        public event TreeViewListEventHandler NodeClicked;
        protected virtual void OnNodeClick(Node node, bool isHeader)
        {
            if (NodeClicked != null)
                NodeClicked(this, new TreeListViewEventArgs(node) { IsHeaderClick = isHeader });
        }
        ...
        {
            if (AfterSelect != null)
                AfterSelect(this, new TreeListViewEventArgs ( node ));
        }
		...
		int m_hotrow = -1;
		public int CalcHitRow(Point mousepoint)
		{
		...
		protected override void OnMouseClick(MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left)
			{
				...
				if (clickedNode != null && Columns.Count > 0)
				{
                    ...
                    if(clickedNode is CheckNode)
                    {
                        var cknode=(CheckNode)clickedNode ;
                       // int wtmp = Columns[0].CalculatedRect.X + Columns.Options.LeftMargin + GetIndentSize(clickedNode) + glyphRect.Width;
                        if (e.X >= glyphRect.Right && e.X <= glyphRect.Right + TextRenderer.MeasureText(cknode[0].ToString(), GetFormatting(clickedNode, Columns[0]).Font).Height + 2)
                        {
                            cknode.IsChecked = !cknode.IsChecked;                           
                            Refresh();
                            cknode.OnCheckClick();
                            base.OnMouseClick(e);
                            return;
                    ...
                    else
                    {
                        ...
                        if (e.X < RowHeaderWidth())
                            OnNodeClick(clickedNode, true);
                        else OnNodeClick(clickedNode, false);
                    }
					if (MultiSelect)
					{
		...
		internal virtual int GetIndentSize(Node node)
		{
			int indent = 0;
			Node parent = node.Parent;
			while (parent != null)
		...
		internal virtual Rectangle GetPlusMinusRectangle(Node node, TreeListColumn firstColumn, int visibleRowIndex)
		{
        ...
        public virtual Image GetRowHeaderBitmap(Node node)
        {
            if (Images != null && node.HeaderImageId >= 0 && node.HeaderImageId < Images.Images.Count)
                return Images.Images[node.HeaderImageId];
            return null;
        }
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			foreach (Node node in NodeCollection.ForwardNodeIterator(m_firstVisibleNode, true))
			{
				...
				headerRect.Width = RowHeaderWidth();
                m_rowPainter.DrawHeader(e.Graphics, headerRect, absoluteRowIndex == m_hotrow, GetRowHeaderBitmap(node));
    ...
    public class TreeListViewEventArgs:EventArgs
    {
        ...
        public bool IsHeaderClick { get; set; }
        public TreeListViewEventArgs(Node node)
        {
            this.Node = node;
            IsHeaderClick = false;
        }
