...
{
    public class ListViewCollapse : ListViewEx
    {
        ...
        public  static void setGrpState(ListViewGroup lstvwgrp, ListViewGroupState state)
        {
            ...
            if (lstvwgrp == null || lstvwgrp.ListView == null)
                return;
            if (lstvwgrp.ListView.IsDisposed)
                return;
            ...
            else
            {
                ...
                IntPtr ptr = Marshal.AllocHGlobal(1024000);
                if (GrpId != null)
                {
                    group.IGroupId = GrpId.Value;

                    Marshal.StructureToPtr(group, ptr, true);  
                 
                    SendMessage(lstvwgrp.ListView.Handle, LVM_SETGROUPINFO, GrpId.Value, ptr);
                  //  SendMessage(lstvwgrp.ListView.Handle, LVM_SETGROUPINFO, GrpId.Value, ptr);
                ...
                else
                {
                    ...
                    group.IGroupId = GrpId.Value;
               //     SendMessage(lstvwgrp.ListView.Handle, LVM_SETGROUPINFO, gIndex, ptr);
                    SendMessage(lstvwgrp.ListView.Handle, LVM_SETGROUPINFO, gIndex, ptr);
                }
        ...
        {
          //new TaskFactory().StartNew(()=> Parallel.ForEach( this.Groups.Cast<ListViewGroup>() ,(lvg)=>
          //      setGrpState(lvg, state)));
            foreach (ListViewGroup lgv in this.Groups)
                setGrpState(lgv, state);
        }
