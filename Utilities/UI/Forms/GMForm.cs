...
namespace Utilities.UI
{
    ...
    public partial class GMForm : Form
    {
        ...
        public event EventHandler OnOptionboxClick { add { controlBoxManager.OnOptionboxClick += value; }
            remove { controlBoxManager.OnOptionboxClick -= value; }
        }
...
//         }
        ...
        {
            get { return (this.ShowIcon ? XTheme.IconTopMargin : 0); }
            set
            {
                XTheme.IconTopMargin = value;
                Invalidate();
            }
        }
        public TextFormatFlags CaptionTextFormat
        {
            get;
            set;
        }

        bool _OptionBox = false;
        [Category("XForm")]
        [Description("是否显示选项按钮")]
        [DefaultValue(typeof(bool), "false")]
        public bool OptionBox
        {
            get { return _OptionBox; }
            set
            {
                _OptionBox = value;
                Invalidate();
        ...
        {
            get { return XTheme.TextTopMargin; }
            set
            {
                XTheme.TextTopMargin = value;
                Invalidate();
            }
        }
        [Category("XForm")]
        ...
        [Category("XForm")]
        [DefaultValue(typeof(Size), "25, 17")]
        public Size OptionBoxSize
        {
            get { return XTheme.OptionBoxSize; }
            set
            {
                XTheme.OptionBoxSize = value;
                Invalidate();
            }
        }
        #endregion
        ...
        public Rectangle BottomLeftRect
        {
            get
            {
                return new Rectangle(
        ...
        public Rectangle BottomRightRect
        {
            get
            {
                return new Rectangle(
        ...
        public Rectangle CaptionRectToDraw
        {
            get
            {
                return new Rectangle(
        ...
        public Rectangle OptionBoxRect
        {
            get
            {
                if (ControlBox && OptionBox)
                {
                    int x = MinBoxRect.Left - ControlBoxSpace - OptionBoxSize.Width;
                    return new Rectangle(
                        new Point(x, ControlBoxOffset.Y), OptionBoxSize);
                }
                else
                    return Rectangle.Empty;
            }
        ...
        public Rectangle IconRect
        {
            get
            {
                if (ControlBox && ShowIcon)
                {
                    ...
                    int y = IconTopMargin.HasValue ? BorderWidth + IconTopMargin.Value : BorderWidth + (CaptionHeight - IconSize.Height) / 2;
        ...
        public Rectangle TextRect
        {
            get
            {
                ...
                int y = BorderWidth+TextTopMargin;
                ...
                {
                    var mr = g.MeasureString(Text, XTheme.CaptionTextFont);
                    if (mr.Width < width)
                    {
                        width = (int)mr.Width + 10;
            ...
            :base()
        {
            InitializeComponent();
            FormIni();
            TextChanged += GMForm_TextChanged;
            // 下面这个条件是永远不为true的，如果不把此类直接设置成mdicontainer
            //if (this.IsMdiContainer)
            //    SetMdiClient();
        }

        void GMForm_TextChanged(object sender, EventArgs e)
        {
        ...
        private void FormIni()
        {
            ...
            CaptionTextFormat = TextFormatFlags.Left | TextFormatFlags.Top;
            if (ControlBox)
            {
        ...
        private void DrawFormIconAndText(Graphics g)
        {
            ...
            if (!string.IsNullOrEmpty(Text) && XTheme.DrawCaptionText)
            {
                TextRenderer.DrawText(
                    g,
                    this.Text,
                   XTheme.CaptionTextFont,
                    TextRect,
                    XTheme.CaptionTextColor,
                    //(XTheme.CaptionTextCenter ? TextFormatFlags.HorizontalCenter : TextFormatFlags.Left) | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis
                    CaptionTextFormat
                    );
            }
        }
        ...
        private bool WmNcHitTest(ref Message m)
        {
            ...
            if (CloseBoxRect.Contains(p) || MaxBoxRect.Contains(p) || MinBoxRect.Contains(p) || OptionBoxRect.Contains(p))
            {
                if (OptionBoxRect.Contains(p))
                    if (toolTip.GetToolTip(this)!=OptionTip)
                    toolTip.SetToolTip(this, OptionTip);
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                case (int)WinAPI.WindowMessages.WM_NCLBUTTONDOWN:
                    break;
...
//         }
        bool IsDesignMode = UIhelper.IsDesignMode();
        /// <summary>
...
//                 {
//                     if (diffx == 4 * 2 || diffx == 8 * 2 || IsDesignMode)
//                     {
//                         x -= diffx;
//                         y -= diffy;
//                     }
