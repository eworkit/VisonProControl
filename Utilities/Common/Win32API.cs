...
namespace Utilities
{
    ...
    public  static class Win32Api
    {
        ...
        public static extern IntPtr SetFocus(IntPtr hWnd);
         [DllImport("user32.dll")]
         public static extern bool ShowWindowAsync(IntPtr hWnd, SWAFlags nCmdShow);
        ...
        public static void ActivateWindow(IntPtr hWnd)
        {
            ...
            if (IsIconic(hWnd))
            {
                ShowWindowAsync(hWnd, SWAFlags.SWA_RESTORE);
            }
            else
            {
                SwitchToThisWindow(hWnd, true);
                return;
                ShowWindowAsync(hWnd, SWAFlags.SWA_SHOWNORMAL);
            }
            return;
        }
        #region Gdi32.dll
     ...
     /*   static bool IsAppStarted0()
        {
          var processes = System.Diagnostics.Process.GetProcessesByName(Path.GetFileNameWithoutExtension(Application.ExecutablePath));// (Application.ProductName);
            foreach (var p in processes)
            {
                if (p.Id != Process.GetCurrentProcess().Id)
                {
                    ...
                    //                     GetWindowText(p.MainWindowHandle, s, s.Capacity);
                    string s = p.MainWindowTitle;
                    if (s == LangsRes.sDataAnalysis || s == LangsRes.sReportSearch || s == LangsRes.sStatisticAnalysis)
                        return false;
                    if (s.Contains(LangsRes.sAppTitle))
                        ShowWindow(p.MainWindowHandle, 3);
                    else
    ...
    {
        SWP_NOSIZE = 0x1, //忽略 cx、cy, 保持大小}
        SWP_NOMOVE = 0x2, //忽略 X、Y, 不改变位置}
        SWP_NOZORDER = 0x4, //忽略 hWndInsertAfter, 保持 Z 顺序}
        SWP_NOREDRAW = 0x8,  //不重绘}
    ...
    [Flags]
    public enum SWAFlags
    {
        SWA_HIDE = 0,
        SWA_SHOWNORMAL = 1,
        SWA_SHOWMINIMIZED = 2,
        SWA_SHOWMAXIMIZED = 3,
        SWA_SHOWNOACTIVATE = 4,
        SWA_RESTORE = 9,
        SWA_SHOWDEFAULT = 10
    }
    /// <summary>
